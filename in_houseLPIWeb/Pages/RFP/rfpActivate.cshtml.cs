...
{
    [Authorize]
    public class rfpActivateModel : PageModel
    {
        private readonly WebDbContext _db;
        private readonly ILogger<rfpActivateModel> _logger;

        public rfpForm rfpFormx { get; set; }
        public PoPList PoPListx { get; set; }
        public IEnumerable<PoPList> PoPLists { get; set; }
        public rfpActivateModel(WebDbContext db, ILogger<rfpActivateModel> logger)
        {
            _db = db;
            _logger = logger;
        }
        ...
        public IActionResult OnGet(int id)
        {
            ...
            var hasPermission = _db.Permissions.Any(p => p.UserId == userInfo.Id && p.RFP_Activate == true);
        ...
        public async Task<IActionResult> OnPost(rfpForm rfpFormx)
        {
            ...
            if (rfpFromDb.IsActive == false)
            {
                rfpFromDb.Status = "Open";
                rfpFromDb.IsActive = true;

                await _db.SaveChangesAsync();
                TempData["Success"] = "Request for Payment activated successfully.";
            }
            else
            {
                TempData["Error"] = "The select Request for Payment is already active.";
                return RedirectToPage("/RFP/rfpIndex");
            }
